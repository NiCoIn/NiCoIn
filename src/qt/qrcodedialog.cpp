...
{
    QString ret = QString("nicoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NCI unit input in lnReqAmount, we generate the URI with NCI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(NiCoInUnits::format(NiCoInUnits::NCI, ui->lnReqAmount->valueAsMpq()));
            paramCount++;
        }
        else
        {
